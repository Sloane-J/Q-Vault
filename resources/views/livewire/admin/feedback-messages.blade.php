<div
    class="flex flex-col p-6 rounded-2xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 shadow-md space-y-6">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div
            class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 shadow-sm p-5">
    ...
    <div
        class="rounded-xl border border-neutral-200 dark:border-neutral-700 bg-white dark:bg-neutral-900 shadow-sm overflow-hidden">
        <!-- Table Header with potential filters -->
        <div
            class="px-5 py-3 bg-neutral-50 dark:bg-neutral-800 border-b border-neutral-200 dark:border-neutral-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        ...
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-neutral-200 dark:divide-neutral-700">
                <thead class="bg-neutral-50 dark:bg-neutral-800">
                    <tr>
                        <th
                            class="px-5 py-3 text-left text-xs font-medium text-neutral-600 dark:text-neutral-400 uppercase tracking-wider">
                            Message</th>
                        <th
                            class="px-5 py-3 text-left text-xs font-medium text-neutral-600 dark:text-neutral-400 uppercase tracking-wider">
                            Category</th>
                        ...
                        <th
                            ...
                            Date</th>
                        <th
                            class="px-5 py-3 text-right text-xs font-medium text-neutral-600 dark:text-neutral-400 uppercase tracking-wider">
                            Actions</th>
                    </tr>
                </thead>
                        ...
                        <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800 transition-colors duration-150">
                            <td class="px-5 py-3">
                                <div class="flex items-center">
                                    <div class="text-sm font-medium text-neutral-900 dark:text-neutral-100 break-words">
                                        {{ $message->message }}
                                    </div>
                                </div>
                            </td>
                            <td class="px-5 py-3 whitespace-nowrap">
                                <span
                                    class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border border-green-200 dark:border-green-700 bg-green-50 dark:bg-green-500/10 text-green-800 dark:text-green-200">
                            ...
                            <td class="px-5 py-3 whitespace-nowrap">
                                <div class="flex flex-col">
                                    <span
                                        class="text-sm font-medium text-neutral-900 dark:text-neutral-100">{{ $this->getUserName($message->user_info) }}</span>
                                    <span
                                        class="text-xs text-neutral-500 dark:text-neutral-400">{{ $this->getIpAddress($message->user_info) }}</span>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-sm text-right" x-data="{ open: false }">
                                <div class="relative inline-block text-left">
                                    <button type="button" @click="open = !open"
                                        :class="{
                                            'bg-red-100 text-red-800 dark:bg-red-900/20 dark:text-red-300': '{{ $message->status }}'
                                            === 'new',
                                            'bg-yellow-100 text-yellow-800 dark:bg-yellow-900/20 dark:text-yellow-300': '{{ $message->status }}'
                                            === 'read',
                                            'bg-green-100 text-green-800 dark:bg-green-900/20 dark:text-green-300': '{{ $message->status }}'
                                            === 'resolved',
                                        }"
                       ...
                       focus:ring-offset-white dark:focus:ring-offset-neutral-900 transition-colors duration-200 ease-in-out">
                                    ...
                                    <div x-show="open" @click.away="open = false"
                                        ...
                                        <div class="py-1" role="menu" aria-orientation="vertical"
                                            ...
                                            <a href="#"
                                                wire:click.prevent="updateStatus({{ $message->id }}, 'new')"
                                                @click="open = false"
                                                class="block px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-700"
                                                role="menuitem">New</a>
                                            <a href="#"
                                                wire:click.prevent="updateStatus({{ $message->id }}, 'read')"
                                                @click="open = false"
                                                class="block px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-700"
                                                role="menuitem">Read</a>
                                            <a href="#"
                                                wire:click.prevent="updateStatus({{ $message->id }}, 'resolved')"
                                                @click="open = false"
                                                class="block px-4 py-2 text-sm text-neutral-700 dark:text-neutral-200 hover:bg-neutral-100 dark:hover:bg-neutral-700"
                                                role="menuitem">Resolved</a>
                                        </div>
                                    </div>
                                </div>
                            </td>
                        ...
                        <tr>
                            <td colspan="6" class="px-5 py-8 text-center text-neutral-500 dark:text-neutral-400">
                                <div class="flex flex-col items-center justify-center">
                                    <svg class="w-12 h-12 text-neutral-400 dark:text-neutral-500 mb-3" fill="none"
