    ...
    <div class="flex flex-col gap-6 md:flex-row md:items-center md:justify-between">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 w-full md:w-auto">
            ...
            <div class="p-5 rounded-xl border border-pink-200 dark:border-pink-700 bg-pink-50 dark:bg-pink-500/10 shadow-sm">
                ...
                <p class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ $activeTodayCount }}</p>
            </div>
            <div class="p-5 rounded-xl border border-red-200 dark:border-red-700 bg-red-50 dark:bg-red-500/10 shadow-sm">
                ...
                <p class="text-2xl font-bold text-neutral-900 dark:text-neutral-100">{{ $newThisWeekCount }}</p>
            </div>
        </div>
    </div>
                ...
                <tr>
                    <th class="px-6 py-4 cursor-pointer" wire:click="sortBy('name')">
                        <div class="flex items-center gap-1">
                            Name
                            @if($sortField === 'name')
                                <svg class="w-3 h-3" fill="none" stroke="currentColor"><path d="{{ $sortDirection === 'asc' ? 'M5 15l7-7 7 7' : 'M19 9l-7 7-7-7' }}" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            @endif
                        </div>
                    </th>
                    ...
                    <tr class="hover:bg-neutral-50 dark:hover:bg-neutral-800/50 transition">
                        <td class="px-6 py-4 whitespace-nowrap">
                            <div class="flex items-center gap-3">
                                ...
                                <div>
                                    <div class="font-medium text-neutral-900 dark:text-neutral-100">{{ $student->name }}</div>
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="inline-flex items-center space-x-2">
                                <button wire:click="editStudent({{ $student->id }})" class="text-blue-600 hover:text-blue-800 dark:text-blue-400 dark:hover:text-blue-300 transition" title="Edit">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                    </svg>
                                </button>
                                <button wire:click="confirmDelete({{ $student->id }})" class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 transition" title="Delete">
                                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
